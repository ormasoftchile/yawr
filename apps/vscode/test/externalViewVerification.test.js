...
const { waitForExternalViewVerification } = require('../out/externalViewVerification');

function harness(post = async () => true) {
  let cancelled = false;
  const messages = new Set(), cancellations = new Set(), sent = [];
  const args = {
    capability: 'external-view.open', runId: 'run', turnId: 'turn', correlationId: 'correlation',
  ...
  return {
    start: () => waitForExternalViewVerification(args, transport), sent,
...
test('real-view verification waits for the matching operator result, not request delivery', async () => {
  ...
  const verified = { ...h.sent[0], type: 'yawr.external-view.view-verified', status: 'opened' };
...
test('operator-reported external view startup failure settles negatively and cleans up listeners', async () => {
  const h = harness();
  const pending = h.start();
  await new Promise(setImmediate);
  h.emit({ ...h.sent[0], type: 'yawr.external-view.view-verified', status: 'failed' });
...
test('cancellation before and during view verification cleans up and suppresses late confirmation', async () => {
  for (const before of [true, false]) {
    ...
    else h.emit({ ...h.sent[0], type: 'yawr.external-view.view-verified', status: 'opened' });
  }
});
