...
const { launchExternalViewWithHandoff, externalViewParameterArguments } = require('../out/externalViewHandoff');
...
async function launch(overrides = {}) {
  ...
  const result = await launchExternalViewWithHandoff(
...
test('void external view command completes only after operator verification of the real view', async () => {
...
test('External view CLI parameters preserve environment, server and database exactly', () => {
  assert.equal(externalViewParameterArguments('ProdEus1a', { server: 'server-859807057', database: 'database-859807057' }),
    '-p environment:ProdEus1a -p server:server-859807057 -p database:database-859807057');
  // Current external view parses values up to the next "-p"; it does not unquote shell strings.
  const args = externalViewParameterArguments('ProdEus1a', { server: 'host\\instance', database: 'Database With Spaces', count: 3 });
...
test('ambiguous CLI parameters are rejected rather than changing the requested environment or values', () => {
  for (const parameters of [{ environment: 'other' }, { server: 'host -p environment:other' },
    ...
    assert.throws(() => externalViewParameterArguments('ProdEus1a', parameters));
  }
});
