...
Dynamic metadata-bearing `execution-flow-closure/v2` retains exact action definitions captured by the existing executable materializer before resolution commit. Readers validate the closure digest and bind its tools through the recorded frame/occurrence/revision; no post-commit catalog fallback. Metadata-free yawr.closure/v1 bytes remain unchanged.
Existing run-state/v2, session frame projections and plan/frame/result histories remain. Replay uses original graph revisions/events; legacy missing metadata renders plain, not current-registry colors.

## 5. Snapshot/toolchain compatibility

Write `execution-plan/v2` **only if presentation occurs anywhere in serialized frozen tools/closures**; metadata-free new plans retain v1 encoding. Existing v1 snapshots/digests remain byte/semantic-verification compatible; never rewrite/upconvert history.
Dual readers strictly accept v1/v2; v1 must reject embedded presentation even after adding Go fields. Version preflight precedes lease/state mutation, then recheck under lease; reject duplicate/unknown versions, trailing data and corrupt digests.
Writer opt-in is the new metadata declaration itself **within the upgraded supported cohort**. Ship dual readers/bundled helper+UI first; enable metadata-bearing definitions only in stores whose participating engines are upgraded. No extra default-off authoring flag.
The extension may author with its bundled helper regardless of an older configured execution binary; execution with metadata requires that matching upgraded bundled runtime or explicit verified dual-reader runtime. Incompatible runtime => actionable failure before start.
Exact verification command: `yawr presentation capabilities` returns `{"schema_version":"yawr.presentation-capabilities/v1","resolver_version":"yawr.core-binding/v1","execution_plan_read":["yawr.execution-plan/v1","execution-plan/v2"],"execution_plan_write":["yawr.execution-plan/v1","execution-plan/v2"]}`. Exit failure/missing capability blocks metadata-bearing execution, not editor highlighting.
Unknown/mixed shared-store cohorts must not enable v2 writers. Untouched old binaries cannot be fixed retroactively and may mutate lease artifacts before rejection; do not promise otherwise.
Tool/provenance/plan digests can change with metadata; dispatched arguments, permissions, capture and interpolation do not. No mandatory checkpoint/run-state/v3.
